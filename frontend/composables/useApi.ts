...
const scriptureSchema = z.object({
  ...
  current_indexing_job: z
    .object({
      ...
      ocr_pages_processed: z.number().int().nonnegative(),
      ocr_checkpoint_pages: z.number().int().nonnegative(),
      is_replaying_checkpoint: z.boolean(),
      volumes_processed: z.number().int().nonnegative(),
      source_volumes: z.number().int().nonnegative(),
